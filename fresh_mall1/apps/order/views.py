...
# /order/place
class OrderPlaceView(LoginReguiredMixin, View):
    """提交订单页面显示"""
...
#             # 用户没登录
#             return JsonResponse({'res': 0, 'errmsg': '用户没登录'})
#
#         # 接受参数
#         addr_id, pay_method, sku_ids = self.accept_parameters(request)
#
#         # 校验参数
...
#                                              transit_price=transit_price)
#
#             # todo： 用户的订单中有几个商品，需要向db_order_goods表中加入几条记录
#             conn = get_redis_connection('default')
#             cart_key = 'cart_%d' % user.id
#
...
#                     # time.sleep(10)
#
#                     # update.db_goods_sku set stock=new_stock, sales = new_sales
#                     # where id=sku_id and stock = orgin_stock
#                     # 返回受影响的行数
#                     # 使用乐观锁来判断数据是否有人更改
#                     res = GoodsSKU.objects.filter(id=sku_id,
#                                                   stock=orgin_stock).update(
#                         stock=new_stock, sales=new_sales)
#                     if res == 0:
#                         if i == 2:
#                             transaction.savepoint_rollback(save_id)
#                             return JsonResponse({'res': 2, 'errmsg': '下单失败2'})
#                         # continue 是在次返回循环
#                         continue
#
...
#
#     def accept_parameters(self, request):
#         addr_id = request.POST.get('addr_id')
#         pay_method = request.POST.get('pay_method')
#         sku_ids = request.POST.get('sku_ids')
#         return addr_id, pay_method, sku_ids
...
class OrderPayView(View):
    ...
    def post(self, request):
        ...
        app_private_key_string = open(os.path.join(settings.BASE_DIR,
                                                   'apps/order/app_private_key.pem')).read()
        alipay_public_key_string = open(os.path.join(settings.BASE_DIR,
                                                     'apps/order/alipay_public_key.pem')).read()
        # 业务处理：使用Python sdk调用支付宝的支付接口
        # 初始化
        alipay = AliPay(
            appid="2021000121607928",  # 应用id
...
class CommentView(LoginReguiredMixin, View):
    """订单评论"""
